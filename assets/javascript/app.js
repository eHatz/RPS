...
dbRef.child('player1').set({
	...
	playerLosses: '',
	playerChoice: ''
}); //every time someone leaves or refreshes the page the values in player 1 and 2 will be reset
...
$('#addUserBtn').on('click', function() {
	if (!valueInP1 && !valueInP2) { 	//if there is currently no player1 and 2 the value entered in the text box will be set to player 1 and stored in the localUser1 var
		...
		dbRef.child('player1').child('playerName').set(localUser1); //resets the value on the database to the value in localUser1
		$('#userNameForm').html('Hi ' + localUser1 + ', you are Player 1!');
		addRpsButtons('player1');
		...
		dbRef.child('player2').child('playerName').set(localUser2); //resets the value on the database to the value in localUser1
		$('#userNameForm').html('Hi ' + localUser2 + ', you are Player 2!');
		addRpsButtons('player2');
		...
		localUser1 = $('#userNameInput').val();
		dbRef.child('player1').child('playerName').set(localUser1);
		$('#userNameForm').html('Hi ' + localUser1 + ', you are Player 1!');
		addRpsButtons('player1');
	};
...
dbRef.on('value', function(snapshot) { 
	valueInP1 = snapshot.val().player1.playerName;
	console.log(valueInP1);
	valueInP2 = snapshot.val().player2.playerName;
	console.log(valueInP2);
	...
	if (localUser1) { //if there is a value in localUser1 change the value of player1 in the database (each player will only have one of the variables, the other 1 will remain undefined locally)
		...
		dbRef.child('player2').child('playerName').set(localUser2);
	}
